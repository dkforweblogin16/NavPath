  ...
  chapter.topics.forEach(topic => {
    ...
    html += `
      <div class="topic-item ${done ? 'completed' : ''} ${isLocked ? 'locked' : ''} ${isActive ? 'active-topic' : ''}"
        ...
        <div class="topic-check" ${!isLocked ? `onclick="event.stopPropagation();toggleTopic('${topic.id}')" style="cursor:pointer;" title="${done ? 'Mark incomplete' : 'Mark complete'}"` : ''}>${done ? '✓' : ''}</div>
...
function openPremiumModal() {
  if (isAdmin()) { toast('🛡️ Admin account — full access already enabled!', 'success'); return; }
